...
export default function PostBox({id, journalEntry, createdBy, text, comments} : PostExt) {
  ...
  return (
    <div className="flex flex-col mx-auto" key={id}>
      <JournalEntryBox {...journalEntry} />
      <div className="flex mt-6">
        {createdBy.image &&
          <Image src={createdBy.image} className="rounded-full" width={50} height={50} alt='user image' />
        }
      ...
      <div className="flex my-4">
        <div className="w-14">
          {session?.user &&
            <Image src={session.user.image} className="rounded-full" width={50} height={50} alt='user image' />
          }
        </div>
        <div className="flex flex-col w-11/12">
          <textarea name="comment" id="comment" className="ml-4 placeholder:italic" placeholder="Add a comment..." value={commentText} onChange={e => setCommentText(e.target.value)}></textarea>
          <div className="flex justify-end mt-2">
            <div className="btn bg-transparent" onClick={handleSubmitComment}>
              Comment
            </div>
          </div>
        </div>
      ...
      {(updatedComments as CommentExt[]).map((comment, index) => {
        return (
          <div className="flex mt-8" key={`comment-${index}`}>
            <div className="w-14">
              {comment.user && comment.user.image &&
                <Image src={comment.user.image} className="rounded-full" width={50} height={50} alt='user image' />
              }
            </div>
            ...
            <div className="flex justify-end ml-auto w-12">
              {comment.userId === session?.user.id && 
                <div className="dropdown dropdown-end">
                  <label tabIndex={0} className="btn bg-transparent p-0">

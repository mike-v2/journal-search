...
const queryModelTemperature = 0.2;

const responseModel = 'gpt-4o-mini';
const responseModelTemperature = 0.2;
...
export async function POST(req: Request) {
  ...
  function pruneString(str: string, maxTokens: number, pruneEnd = true) {
    ...
    if (numTokens < maxTokens) {
      ...
      return pruneEnd
        ? str.substring(0, newLength)
        : str.substring(str.length - newLength);
    }
  }
  ...
  async function downloadEmbeddings() {
    const csvData = await downloadBlob(
      'journal_entries_harry_howard',
      'journal-entries-with-embeddings.csv',
    );
    if (csvData) {
  ...
  async function findEntriesRelatedToMessage(
    query: string,
    n = 20,
  ...
  console.log(
    `downloaded ${embeddings.length} embeddings. first entry date: ${embeddings[0].date}`,
  );
  ...
  try {
    ...
    const response: string = fullResponse['choices'][0]['message'][
      'content'
    ] as string;
    ...
    console.log(
      'message history length (including system message): ' + messages.length,
    );

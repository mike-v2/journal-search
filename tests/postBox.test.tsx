...
describe('PostBox rendering', () => {
  ...
  test("displays user's avatar correctly", () => {
    ...
    expect(
      getByAltText(`${mockPost.createdBy.name}'s avatar`),
    ).toBeInTheDocument();
    expect(getByAltText(`${mockPost.createdBy.name}'s avatar`)).toHaveAttribute(
      'src',
      expect.stringMatching(/via\.placeholder/),
    );
  });
...
describe('PostBox interaction', () => {
  ...
  test('fills the new text area with the original comment when user clicks edit', () => {
    const specialComments: CommentExt[] = [
      {
        id: 'comment-id',
    ...
    const { getByText, getAllByLabelText } = render(
      <SessionProvider session={mockSession}>
        <PostBox {...mockPost} comments={specialComments} />
      </SessionProvider>,
    );
    // expand post
    fireEvent.click(getByText(mockPost.title));
  ...
  test('handles cancelling comment edit', async () => {
    ...
    const { getByText, getAllByLabelText, queryByText, findByText } = render(
      <SessionProvider session={mockSession}>
        <PostBox {...mockPost} comments={specialComments} />
      </SessionProvider>,
    );

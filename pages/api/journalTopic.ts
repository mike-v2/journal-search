    ...
    //get topics for journal entry
      ...
      try {
        ...
        if (!topics) {
          return res
            .status(404)
            .json({ error: 'Topics not found for entry ' + journalEntryId });
        }
        ...
        res
          .status(500)
          .json({ error: 'An error occurred while fetching topic data' });
      }
      ...
      try {
        ...
        if (!topic) {
          return res
            .status(404)
            .json({ error: 'Topic not found for topicId ' + topicId });
        }
      ...
      try {
        ...
        res
          .status(500)
          .json({ error: 'An error occurred while fetching topics data' });
      }
    }
  }
}

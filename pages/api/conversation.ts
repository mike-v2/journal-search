...
export default async function handler(
  ...
  if (req.method === 'GET') {
    ...
    //get all conversations for user
      ...
      try {
        ...
        if (!conversations) {
          return res
            .status(404)
            .json({ error: 'Conversations not found for user ' + userId });
        }
      ...
      try {
        ...
        if (!conversation) {
          return res
            .status(404)
            .json({ error: 'Messages not found for conversation ' + id });
        }
    ...
    if (!id || !title) {
      return res
        .status(404)
        .json({ error: 'id and title are required to patch a conversation' });
    }
